...
export default function FlightSearchWidget() {
  ...
  const search = () => {
    let msg = `✈️ *SOLICITUD DE COTIZACIÓN*\n\n`;
    ...
    window.open(`https://wa.me/18296640750?text=${encodeURIComponent(msg)}`, '_blank');
  };
  ...
  const handleServiceTab = (tab: ServiceTab) => {
    if (tab !== 'vuelos') {
      ...
      window.open(`https://wa.me/18296640750?text=${encodeURIComponent(`Hola! Me interesa información sobre ${labels[tab]}.`)}`, '_blank');
    }
    setServiceTab(tab);
  };
